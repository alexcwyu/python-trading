...
class SampleFactory(object):
    ...
    def sample_instrument(self):
        inst = ModelFactory.new_instrument(symbol='2800.HK', type=Instrument.STK, primary_exch_id='SEHK', ccy_id='HKD',
    ...
    def sample_exchange(self):
        exchange = ModelFactory.new_exchange('SEHK', 'The Stock Exchange of Hong Kong Limited', 'HK', 'HKEX',
                                             'HK_Holiday')
        return exchange

    def sample_currency(self):
        currency = ModelFactory.new_currency('HKD', 'Hong Kong Doller')
        return currency

    def sample_country(self):
        country = ModelFactory.new_country('US', 'United States of America', 'US_holiday')
        return country

    def sample_trading_holidays(self):
        trading_holiday = ModelFactory.new_holiday_series("HK holiday")
    ...
    def sample_trading_hours(self):
        trading_hours = ModelFactory.new_trading_hours(trading_hours_id='SEHK_trdinghr', timezone_id='HK timezone'
                                                       )
    ...
    def sample_timezone(self):
        timezone = ModelFactory.new_timezone("Venezuela Standard Time")
        return timezone

    def sample_time_series(self):
        ds = ModelFactory.new_time_series("HSI.BAR.86400", name="HSI.BAR.86400", desc="HSI", inputs=["HSI.BAR.1"],
    ...
    def sample_bar(self):
        bar = ModelFactory.new_bar("HSI@SEHK", Bar.Time, 86400, provider_id='IB', timestamp=12312,
    ...
    def sample_quote(self):
        quote = ModelFactory.new_quote("HSI@SEHK", provider_id="IB", timestamp=12312,
                                       utc_time=12312, bid=50, bid_size=34, ask=102.2, ask_size=1)
        return quote

    def sample_trade(self):
        trade = ModelFactory.new_trade("HSI@SEHK", provider_id="IB", timestamp=12312,
                                       utc_time=12312, price=123, size=123)
        return trade

    def sample_market_depth(self):
        md = ModelFactory.new_market_depth("HSI@SEHK", provider_id="IB", timestamp=12312, md_provider="H", position=0,
    ...
    def sample_new_order_request(self):
        req = ModelFactory.new_new_order_request(0, "BuyLowSellHigh", "1", portf_id="TestPortf",
    ...
    def sample_order_replace_request(self):
        req = ModelFactory.new_order_replace_request(0, "BuyLowSellHigh", "1", "2", type=Limit, qty=4954.1,
                                                     limit_price=123.2, stop_price=123.2, tif=DAY, oca_tag="23",
                                                     params={"testparam1": "1", "testparam2": "2"})
        return req

    def sample_order_cancel_request(self):
        req = ModelFactory.new_order_cancel_request(0, "BuyLowSellHigh", "1", "2",
                                                    params={"testparam1": "1", "testparam2": "2"})

        return req

    def sample_order_status_update(self):
        event = ModelFactory.new_order_status_update(0, "IB", "event_123", broker_ord_id="broker_1234",
    ...
    def sample_execution_report(self):
        event = ModelFactory.new_execution_report(1, "IB", "event_123", broker_ord_id="broker_1234",
    ...
    def sample_account_update(self):
        event = ModelFactory.new_account_update(0, "IB", broker_event_id="e_123", account_name="account1")
    ...
    def sample_portfolio_update(self):
        event = ModelFactory.new_portfolio_update(0, "IB", broker_event_id="e_456", portf_id="BLSH", inst_id="HSI@SEHK",
    ...
    def sample_account_state(self):
        account = ModelFactory.new_account_state("test_acct")
    ...
    def sample_portfolio_state(self):
        portfolio = ModelFactory.new_portfolio_state("test_portf", cash=1000)
        self.add_sample_position(portfolio)
        return portfolio

    def sample_strategy_state(self):
        strategy = ModelFactory.new_strategy_state("BLSH", config_values={"k1": "v1", "k2": "v2"})

        self.add_sample_position(strategy)
        return strategy

    def sample_order_state(self):
        order = ModelFactory.new_order_state("BuyLowSellHigh", "1", portf_id="TestPortf",
    ...
    def sample_sequence(self):
        seq = ModelFactory.new_sequence("test_seq", 999)
        return seq
