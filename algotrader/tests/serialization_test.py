...
if __name__ == "__main__":
    ...
    to = timeit.Timer(
        "dumps()",
        "from algotrader.tests.serialization_test import thrash_dumps;dumps=thrash_dumps('json')")
    ...
    for x in tmodules:
        try:
            tx = timeit.Timer(
                "dumps()",
                "from algotrader.tests.serialization_test import thrash_dumps;dumps=thrash_dumps('%s')" % x)
    ...
    to = timeit.Timer(
        "loads()",
        "from algotrader.tests.serialization_test import thrash_loads;loads=thrash_loads('json')")
    to =to.timeit(number=PERMUTATIONS)
    ...
    for x in tmodules:
        try:
            tx = timeit.Timer(
                "loads()",
                "from algotrader.tests.serialization_test import thrash_loads;loads=thrash_loads('%s')" % x)
            tx = tx.timeit(number=PERMUTATIONS)
            m = sys.modules[x]
            print_round(x, tx, to)
        except Exception as e:
