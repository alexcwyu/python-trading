...
from algotrader.event.order import Order, OrdAction, OrdType, OrdStatus, OrderStatusUpdate, ExecutionReport
...
class OrderTest(TestCase):
    def test_is_done(self):
        ...
        status_update1 = OrderStatusUpdate(ord_id=1, instrument="HSI", status=OrdStatus.REJECTED)
        ...
        status_update2 = OrderStatusUpdate(ord_id=2, instrument="HSI", status=OrdStatus.CANCELLED)
        order2.update_status(status_update2)
        self.assertTrue(order2.is_done())

        order3 = Order(ord_id=3, instrument="HSI", action=OrdAction.BUY, type=OrdType.LIMIT, qty=1000, limit_price=18.5)
        status_update3 = OrderStatusUpdate(ord_id=3, instrument="HSI", status=OrdStatus.FILLED)
    ...
    def test_leave_qty(self):
        ...
        er1 = ExecutionReport(ord_id=1, er_id=1, instrument="HSI", filled_qty=800, filled_price=18.4,
                              status=OrdStatus.PARTIALLY_FILLED)
        order.add_exec_report(er1)
        self.assertEqual(200, order.leave_qty())

        er2 = ExecutionReport(ord_id=1, er_id=2, instrument="HSI", filled_qty=200, filled_price=18.4,
    ...
    def test_update_status(self):
        ...
        status_update = OrderStatusUpdate(ord_id=1, instrument="HSI", status=OrdStatus.SUBMITTED)
    ...
    def test_update_status_with_diff_ord_id(self):
        ...
        status_update = OrderStatusUpdate(ord_id=2, instrument="HSI", status=OrdStatus.SUBMITTED)
    ...
    def test_exec_report_with_diff_ord_id(self):
        ...
        er1 = ExecutionReport(ord_id=2, er_id=1, instrument="HSI", filled_qty=800, filled_price=18.4,
                              status=OrdStatus.PARTIALLY_FILLED)
    ...
    def test_exec_report_with_exceed_qty(self):
        ...
        er3 = ExecutionReport(ord_id=1, er_id=3, instrument="HSI", filled_qty=100, filled_price=18.4,
                              status=OrdStatus.FILLED)
        with self.assertRaises(Exception) as ex:
