...
class OrderManager(OrderEventHandler, ExecutionEventHandler, MarketDataEventHandler):
    def __init__(self):
        ...
        self.__orders = {}
        self.__orders_by_client = defaultdict(dict)
        self.started = False
    ...
    def send_order(self, new_ord_req):
        if new_ord_req.cl_ord_id in self.__orders_by_client[new_ord_req.cl_id]:
            raise Exception(
                "ClientOrderId has been used!! cl_id = %s, cl_ord_id = %s" % (new_ord_req.cl_id, new_ord_req.cl_ord_id))

        new_ord_req.ord_id = self.next_ord_id()
        order = Order(new_ord_req)
        self.__orders[order.ord_id] = order
        self.__orders_by_client[order.cl_id][order.cl_ord_id] = order
        broker_mgr.get(order.broker_id).on_new_ord_req(new_ord_req)
        return order

    def cancel_order(self, ord_cancel_req):
        if ord_cancel_req.cl_ord_id not in self.__orders_by_client[ord_cancel_req.cl_id]:
            raise Exception("ClientOrderId not found!! cl_id = %s, cl_ord_id = %s" % (
            ord_cancel_req.cl_id, ord_cancel_req.cl_ord_id))

        order = self.__orders_by_client[ord_cancel_req.cl_id][ord_cancel_req.cl_ord_id]

        ord_cancel_req.ord_id = order.ord_id
    ...
    def replace_order(self, ord_replace_req):
        if ord_replace_req.cl_ord_id not in self.__orders_by_client[ord_replace_req.cl_id]:
            raise Exception("ClientOrderId not found!! cl_id = %s, cl_ord_id = %s" % (
            ord_replace_req.cl_id, ord_replace_req.cl_ord_id))

        order = self.__orders_by_client[ord_replace_req.cl_id][ord_replace_req.cl_ord_id]

        ord_replace_req.ord_id = order.ord_id
