...
class PortfolioTest(TestCase):
    ...
    def test_on_exec_report(self):
        ...
        self.assertEqual(PartiallyFilled, order1.status())
        ...
        self.assertEqual(Filled, order1.status())
        ...
        self.assertEqual(expected_cash, self.portfolio.cash())
        self.assertEqual(expected_stock_value, self.portfolio.performance.now('stock_value'))
        self.assertEqual(expected_total_equity, self.portfolio.performance.now('total_equity'))
    ...
    def check_order(self, portfolio, ord_reqs, qtys):
        ...
        self.assertEqual(len(expected_position), len(portfolio.positions()))
        for inst_id, pos_ord_reqs in expected_position.items():
            ...
            for pos_ord_req in pos_ord_reqs:
                self.assertTrue(
                    ModelFactory.build_cl_ord_id(pos_ord_req.cl_id, pos_ord_req.cl_ord_id) in position.orders)
