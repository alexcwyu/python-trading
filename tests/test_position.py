...
class PositionTest(TestCase):
    def test_add_order(self):
        ...
        order2 = Order(ord_id=2, inst_id=1, action=OrdAction.BUY, type=OrdType.LIMIT, qty=1000, limit_price=18.5)
        position.add_order(order2)

        self.assertEquals(2000, position.size)
        self.assertEquals(2, len(position.orders))

        order3 = Order(ord_id=3, inst_id=1, action=OrdAction.SELL, type=OrdType.LIMIT, qty=1200,
                       limit_price=18.5)
    ...
    def test_add_order_with_same_ord_id(self):
        position = Position(1)
        order1 = Order(ord_id=1, inst_id=1, action=OrdAction.BUY, type=OrdType.LIMIT, qty=1000, limit_price=18.5)
        order2 = Order(ord_id=1, inst_id=1, action=OrdAction.BUY, type=OrdType.LIMIT, qty=1000, limit_price=18.5)
    ...
    def test_add_order_with_diff_inst(self):
        position = Position(1)
        order1 = Order(ord_id=1, inst_id=1, action=OrdAction.BUY, type=OrdType.LIMIT, qty=1000, limit_price=18.5)
        order2 = Order(ord_id=2, inst_id=2, action=OrdAction.BUY, type=OrdType.LIMIT, qty=1000,
                       limit_price=18.5)
    ...
    def test_fill_qty(self):
        ...
        order2 = Order(ord_id=2, inst_id=1, action=OrdAction.SELL, type=OrdType.LIMIT, qty=1200,
                       limit_price=18.5)
        position.add_order(order2)
        self.assertEquals(1000, position.filled_qty())
